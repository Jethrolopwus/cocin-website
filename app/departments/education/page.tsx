    ...
    <main className="w-full">
      <section className="bg-white py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-3">Education Department</h1>
          <p className="text-gray-500 max-w-2xl leading-relaxed">Overseeing schools, seminaries, and academic institutional development across COCIN.</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 overflow-x-auto pb-4 scrollbar-hide">
            {departments.map((dept) => (
              <Link key={dept.slug} href={`/departments/${dept.slug}`} className={`flex-shrink-0 px-4 py-2.5 rounded-full text-sm font-medium transition-colors ${dept.slug === "education" ? "bg-[#2563EB] text-white" : "bg-white text-gray-700 border border-gray-300 hover:bg-gray-50"}`}>{dept.label}</Link>
            ))}
          </div>
        </div>
      </section>
      <section className="bg-[#F5F3EF] py-16 sm:py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-500 max-w-md mx-auto leading-relaxed">Detailed information about the Education Department is being compiled and will be available shortly.</p>
        </div>
      </section>
    </main>
  );
